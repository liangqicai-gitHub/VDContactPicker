...
public protocol VDContactPickerDelegate: class {
...
public extension VDContactPickerDelegate {
    func contactPicker(_ contactPicker: VDContactPicker, didContactFetchFailed error: NSError) { }
    func contactPicker(_ contactPicker: VDContactPicker, didCancel error: NSError) { }
    func contactPicker(_ contactPicker: VDContactPicker, didSelectContact contact: VDContact) { }
    func contactPicker(_ contactPicker: VDContactPicker, didSelectMultipleContacts contacts: [VDContact]) { }
...
typealias VDContactHandler = (_ contacts : [CNContact] , _ error : NSError?) -> Void
...
open class VDContactPicker: UIViewController, UITableViewDelegate, UITableViewDataSource, UISearchBarDelegate {
    ...
    public private(set) var selectedContacts = [VDContact]()
    private var filteredContacts = [CNContact]()

    /// The `delegate` for the picker.
    open weak var contactDelegate: VDContactPickerDelegate?
    ...
    public init(delegate: VDContactPickerDelegate? = nil,
    ...
    private func getContacts(_ completion:  @escaping VDContactHandler) {
    ...
    open func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "Cell", for: indexPath) as! VDContactCell
        cell.accessoryType = UITableViewCellAccessoryType.none
        //Convert CNContact to Contact
        let contact: VDContact

        if let searchText = searchBar.text, !searchText.isEmpty {
            contact = VDContact(contact: filteredContacts[(indexPath as NSIndexPath).row])
            ...
            contact = VDContact(contact: contactsForSection[(indexPath as NSIndexPath).row])
        }
    ...
    open func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let cell = tableView.cellForRow(at: indexPath) as! VDContactCell
...
//            self.contactDelegate?.contactPicker(self, didSelectContact: selectedContact)
    ...
    @objc func onTouchCancelButton() {
        // TODO: Set up errors
        contactDelegate?.contactPicker(self, didCancel: NSError(domain: "VDContactsPickerErrorDomain",
                                                                code: 2,
